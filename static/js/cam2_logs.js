...
let userID = 0;
let tbody = document.getElementById('tbody_cam2');

function CameraLogs2(date, time, camera, type) {
...
function AddAllItemsToTable(TheUser) {
    ...
    TheUser.reverse().forEach(element => {
        ...
        CameraLogs2(element.date, element.time, camera, element.type);
    });

    // Clear existing DataTable instance if it already exists
    if ($.fn.DataTable.isDataTable('#cameralogs2')) {
        $('#cameralogs2').DataTable().clear();
        $('#cameralogs2').DataTable().destroy();
    }

    // Initialize DataTable without export buttons
    $(document).ready(function () {
        $('#cameralogs2').DataTable();
    });
}
...
function GetAllDataOnce() {
    ...
    if (user) {
        ...
        const dbRef = ref(db, `Registered_Accounts/${uid}/Detection/CAMERA 2`);

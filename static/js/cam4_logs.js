...
let userID = 0;
let tbody = document.getElementById('tbody_cam4');

function CameraLogs4(date, time, camera, type) {
...
function AddAllItemsToTable(TheUser) {
    ...
    TheUser.reverse().forEach(element => {
        ...
        CameraLogs4(element.date, element.time, camera, element.type);
    });

        // Clear existing DataTable instance if it already exists
        if ($.fn.DataTable.isDataTable('#cameralogs4')) {
            $('#cameralogs4').DataTable().clear();
            $('#cameralogs4').DataTable().destroy();
        }
    
        // Initialize DataTable without export buttons
        $(document).ready(function () {
            $('#cameralogs4').DataTable();
        });
}
...
function GetAllDataOnce() {
    ...
    if (user) {
        ...
        const dbRef = ref(db, `Registered_Accounts/${uid}/Detection/CAMERA 4`);
